...
В приложении созданы следующие модели:
- User - модель пользователь. Содержит поля email, city, phone.
...
В приложении реализованы следующие вспомогательные функции:
1. **** - функция для .
Используется в контроллере для .

### Сериализация<a id="users_serialize"></a>

Реализованы следующие сериализации:
1. UserSerializer - сериализатор для модели User. В Meta класс предоставлен доступ доступ к полям: .

### Классы разрешений<a id="users_permissions"></a>

Реализованы следующие разрешения:
1. IsOwner - проверяет, что пользователь является владельцем . Если владелец - возвращает True, иначе 
False.
